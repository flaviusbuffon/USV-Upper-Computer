...
class sysnthesisGPSSignalSenderData(threading.Thread):
    revData = list()
    counter = 0
    delay = 0
    sumUp = 0
    data_initial_received_flag = 0
    ...
    def __init__(self,RingBuff,Ship,ValidShip,freq,systemFlag):
        ...
        self.delay = 1/freq
        self.systemFlag = systemFlag
        for cnt in range(12):#初始化列表
            self.revData.append(0)
        print("GPS Signal Sender Protocol Data Frame Length Configuration：%d"%len(self.revData))
    def run(self):
        print('GPS Signal Sender Serial Preparing')
        self.ValidShip.manual_automatic = STATE_MANUAL_CTRL
        #用此标志位指示GPS端是否掉线，STATE_MANUAL_CTRL表示在线，STATE_COLLECTING_POINTS表示掉线
        while self.systemFlag:
            time.sleep(self.delay)
            if(self.data_initial_received_flag == 1):
                self.current_time = time.time()
                self.ValidShip.manual_automatic = STATE_MANUAL_CTRL
                if(self.current_time - self.registered_time > 5):
                    print('GPS信号发射端离线！')
                    self.ValidShip.manual_automatic = STATE_COLLECTING_POINTS
            if(self.ObjRingBuff.getLength()>=12):
                self.data_initial_received_flag = 1
                self.data_received_flag = 1
                self.registered_time = time.time()
                for cnt in range(12):#为一帧数据的长度
                    self.revData[0] = self.ObjRingBuff.outputData()
                    if(self.revData[0]==0x73):
                        self.revData[1] = self.ObjRingBuff.outputData()
                        if(self.revData[1]==0x63):
            ...
            if(self.counter == 1):
                ...
                self.sumUp = self.sumUp & 0xFF
                #print("Sum up is calculated to be %d"%self.sumUp)
                if self.sumUp == self.revData[11]:#校验和验证通过
                        #准备转化数据
                    threadLock.acquire()
                    self.Ship.longtitude = bytes()
                    self.Ship.lattitude = bytes()
                    ...
                    if(self.Ship.yaw!=b'00' and self.Ship.pitch!=b'00' and self.Ship.roll!=b'00'):
                        #print(0xc0 & int.from_bytes(bytes([self.revData[20]]),byteorder='big',signed=False))
                        self.ValidShip.fixtype = (0xc0 & int.from_bytes(bytes([self.revData[10]]),byteorder='big',signed=False)) >> 6
                        self.ValidShip.lattitude = (int.from_bytes(self.Ship.lattitude,byteorder='big',signed=False)/1e7)
                        self.ValidShip.longtitude = (int.from_bytes(self.Ship.longtitude,byteorder='big',signed=False)/1e7)
                        self.ValidShip.dataProcessed=1
                        #数据转化完毕
