...
LoRa_ser = serial.Serial("/dev/ttyUSB0",baudrate)
VEHECLE_LENGTH = 150 #载具的长度，单位为cm
print("STM32 Serial Port Configuration:")
print(ser)
if ser.isOpen == False:
    ser.open()
print("GPS Signal Sender Serial Port Configuration:")
print(LoRa_ser)
if LoRa_ser.isOpen == False:
    LoRa_ser.open()
...
LoRa_RingBuff = Comm.ringBuff(50,baudrate,wirelessPort,LoRa_ser)
#创建多线程
tSynthesis = Comm.sysnthesisData(RingBuff,Ship,ValidShip,10,systemOnline)
tUART = Comm.USARTinData(RingBuff,systemOnline)

GPSSignalShip = Comm.UASattitude()
GPSSignalValidShip = Comm.UASattitude()

tLoRaUART = Comm.USARTinData(LoRa_RingBuff,systemOnline)
tLoRaSynthesis = Comm.sysnthesisGPSSignalSenderData(LoRa_RingBuff,GPSSignalShip,GPSSignalValidShip,10,systemOnline)
#创建线程池
THREADS = [tSynthesis,tUART,tLoRaUART,tLoRaSynthesis]
try:
        ...
        if(ValidShip.manual_automatic == STATE_NORMALLY_RUNNING):          
            ...
            while ValidShip.manual_automatic == STATE_NORMALLY_RUNNING:
                if ValidShip.dataProcessed==1:
                    time.sleep(delay)
                    if(GPSSignalValidShip.manual_automatic == STATE_COLLECTING_POINTS):#GPS信号发射端掉线，则立即前往
                        if (Comm.Ship_Attitude_On_Screen(ValidShip)):
                            distance = AziFromPos.distanceFromCoordinate(ValidShip.longtitude,ValidShip.lattitude,GPSSignalValidShip.longtitude,GPSSignalValidShip.lattitude)
                            if distance >= 250:
                                distance = 250
                            angle =  AziFromPos.angleFromCoordinate(ValidShip.longtitude,ValidShip.lattitude,GPSSignalValidShip.longtitude,GPSSignalValidShip.lattitude)
